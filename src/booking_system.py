...
class SlotUnavailableError(Exception):
    ...
    def __init__(self, message: str, conflicting_slots: List[Tuple[datetime, datetime]] = None):
...
class TurfBooking:
    ...
    def _find_conflicting_slots(self, requested_start: datetime, requested_end: datetime) -> List[Tuple[datetime, datetime]]:
        """
    ...
    def is_slot_available(self, requested_start: datetime) -> bool:
        ...
        if conflicting_slots:
            conflict_details = ", ".join([f"({start} to {end})" for start, end in conflicting_slots])
